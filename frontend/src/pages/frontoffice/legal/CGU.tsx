...
export default function CGU() {
  ...
  const s = (key: string) => t(`legal.cgu.${key}`)
  const arr = (key: string) => t(`legal.cgu.${key}`, { returnObjects: true }) as string[]
  ...
  return (
    <LegalLayout title={s('title')}>
      ...
      <LegalFooter>
        {t('legal.contactFooter')} <Accent>{t('legal.contactEmail')}</Accent>
      </LegalFooter>
    </LegalLayout>
  )
}
