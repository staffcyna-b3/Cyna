            ...
            <div className="flex-1 p-4 flex flex-col justify-between">
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center bg-white border border-gray-200 rounded-full">
                        ...
                        <button
                            className="w-7 h-7 flex items-center justify-center text-gray-500 hover:bg-gray-50 rounded-full text-base transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                            onClick={() => onUpdateQuantity(item.id, item.quantity + 1)}
                            disabled={!item.isService && item.stock !== undefined && item.quantity >= item.stock}
                        >
                            +
                        </button>
                    </div>
